...
export async function getYoutubePerformanceDashboard(
  ...
  try {
    const [channels, latestMonth, latestSync] = await Promise.all([
      getManagedChannels(supabase),
      getLatestMetricMonth(supabase),
...
export async function getYoutubeComparisonDashboard(
  ...
  try {
    const [channels, latestSync] = await Promise.all([getManagedChannels(supabase), getLatestSyncRun(supabase)]);
    filters.channelId = resolveSelectedChannelId(filters.channelId, channels);
...
async function getLatestMetricMonth(supabase: ReturnType<typeof createSupabaseAdminClient>) {
  const { data, error } = await supabase
